...
export function WelcomeEntryDialog({ open, onOpenChange }: WelcomeEntryDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent 
        className="sm:max-w-lg p-0 gap-0 overflow-hidden max-h-[90vh] mx-3 sm:mx-auto"
      ...
      >
        ...
        <div className="bg-gradient-to-br from-wellness-lilac/20 to-wellness-sage/20 p-4 sm:p-6 text-center border-b">
          <Logo size="lg" className="mx-auto mb-3 sm:mb-4" />
          <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-1.5 sm:mb-2 px-2 break-words">
        ...
        <div className="p-3 sm:p-4 space-y-2.5 sm:space-y-3 max-h-[55vh] sm:max-h-[60vh] overflow-y-auto">
          {entryOptions.map((option) => {
            ...
            return (
              <button
                ...
                className={`w-full p-3 sm:p-4 rounded-xl border text-left transition-all hover:shadow-md active:scale-[0.98] group ${
                  isAccent 
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform ${
                    isAccent ? "bg-accent/20" : "bg-primary/20"
                  }`}>
                    <Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${isAccent ? "text-accent" : "text-primary"}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className={`text-sm sm:text-base font-semibold text-foreground mb-0.5 sm:mb-1 transition-colors break-words ${
                  ...
                  <ArrowRight className={`w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground shrink-0 group-hover:translate-x-1 transition-all ${
                    isAccent ? "group-hover:text-accent" : "group-hover:text-primary"
                  }`} />
                </div>
              </button>
        ...
        <div className="p-3 sm:p-4 pt-2 border-t bg-muted/30">
